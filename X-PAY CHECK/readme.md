# X-PAY CHECK — RYB Refactor vNext

Refactor totale di `RYB_v20_Zero_Stress` basato su `RYB_Mobile_TechSpec_v1.md`.

## Cosa è stato aggiornato

- Nuova cartella progetto: `X-PAY CHECK`
- Backend riallineato alla TechSpec con endpoint:
  - `auth` (magic-link, social, refresh)
  - `license` (status, validate, restore)
  - `knowledge` (manifest, download, diff)
  - `backup` (checkpoint, Pro-gated)
  - `audit/run` per analisi cedolino
- Motore audit dedicato (`backend/src/services/auditEngine.ts`) con regole base in stile DAG e output strutturato.
- Tipi dominio audit introdotti in `backend/src/types/audit.ts` e `shared/types/index.ts`.
- UI mobile rifocalizzata su esperienza audit-first con semafori e riepilogo anomalie.
- Schema DB aggiornato alla specifica (`infrastructure/docker/init/01-schema.sql`) con tabelle:
  - `users`, `licenses`, `backup_metadata`, `knowledge_releases`, `audit_log`

## Nota ambiente locale

La build automatica non è stata completata in questa sessione per incompatibilità del binario Node locale con macOS Big Sur:

`dyld: Symbol not found ... /usr/local/bin/node (built for Mac OS X 13.5)`

Serve usare una versione Node compatibile con il sistema locale prima di eseguire `npm run build`.
